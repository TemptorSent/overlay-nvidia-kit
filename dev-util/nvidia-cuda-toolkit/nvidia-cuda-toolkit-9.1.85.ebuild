...
IUSE="debugger doc eclipse profiler +gcc-version-hack"

DEPEND=""
RDEPEND="${DEPEND}
	>=sys-devel/gcc-4.7[cxx]
	!gcc-version-hack? ( <sys-devel/gcc-7[cxx] )
...
src_prepare() {
	...
	# Remove bailout version check to support gcc 7.3
	if use gcc-version-hack ; then 
		cuda_supported_gcc=+"7.2 7.3"
...
pkg_postinst() {
	# Fix nvcc complaints with __float128
	if use gcc-version-hack ; then
		grep -q "__CUDACC__" "${EPREFIX}/usr/include/bits/floatn.h" \
			|| sed -e 's/^# \(define __HAVE_FLOAT128\) 1$/# ifdef __CUDACC__\n#  \1 0\n# else\n#  \1 1\n# endif/' \
				-i "${EPREFIX}/usr/include/bits/floatn.h" \
			|| die "Couldn't apply gcc-version-hack to ${EPREFIX}/usr/include/bits/floatn.h"
	fi
...
pkg_prerm() {
	...
	if use gcc-version-hack ; then
		grep -q "__CUDACC__" "${EPREFIX}/usr/include/bits/floatn.h" \
			&& sed -e '/# ifdef __CUDACC__/,/#endif/ { d ; a# define __HAVE_FLOAT128 1 }' \
				-i "${EPREFIX}/usr/include/bits/floatn.h" \
			|| die "Couldn't revert gcc-version-hack in ${EPREFIX}/usr/include/bits/floatn.h"
	fi
}
